...
#### 🎯 Target Variable
- **Loss Given Default (LGD)** is used as the target variable.  
- LGD represents the proportion of loan exposure that is not recovered after default.  
- It is calculated using discounted recovery cash flows to reflect the time value of money.  
- The target is continuous and bounded between 0 and 1.
...
After evaluating predictive accuracy, discriminatory power, and model stability, WOE Logistic Regression was selected as the most suitable model for LGD estimation. The model shows consistently strong performance across all key evaluation dimensions. It provides a good balance between accuracy and practical usability.

The model outperformed alternatives in Out-of-Time (OOT) validation metrics. It achieved the lowest RMSE, MAE, and MAPE, indicating superior prediction accuracy. It also recorded the highest AUC and Gini, showing better ability to distinguish between high- and low-loss cases.

In terms of robustness, the model achieved the lowest Population Stability Index (PSI). This indicates greater resistance to population shifts over time. Such stability is critical in credit risk modeling, where data distributions often change.

Furthermore, the model shows a relatively low average prediction error. The MAE of approximately 5.8% represents the average LGD estimation error, meaning predictions deviate from actual LGD by about 5.8 percentage points on average. The RMSE of around 7.6% reflects larger deviations in certain cases and captures tail risk exposure. Together, these results indicate both strong accuracy and controlled variability.

LightGBM demonstrated competitive predictive capability, especially in capturing non-linear relationships. However, it showed higher PSI values, suggesting weaker stability under changing data distributions. In addition, its black-box nature limits interpretability and explainability.

This lack of transparency makes LightGBM less suitable for regulatory environments. Credit risk models require clear justification, auditability, and interpretability. These requirements are difficult to meet with highly complex models.

In comparison, WOE Logistic Regression provides high interpretability. Variable transformations are monotonic and economically meaningful. This supports easier validation, monitoring, and communication with stakeholders.

Overall, WOE Logistic Regression offers the best trade-off between accuracy, stability, and interpretability. It is well aligned with regulatory expectations. Therefore, it was selected as the final model for deployment.
